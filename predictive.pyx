...
from libc.math cimport INFINITY, sqrt, log, M_PI
cimport cython
from scipy.integrate import dblquad
from scipy.special import logsumexp

def log_numerical_predictive(np.ndarray events, m_min, m_max, sigma_min, sigma_max):
...
cdef double _log_numerical_predictive(np.ndarray events, double m_min, double m_max, double sigma_min, double sigma_max):
    cdef double I, dI
...
cdef inline double log_norm(double x, double x0, double sigma1, double sigma2):
    return -((x-x0)**2)/(2*(sigma1**2 + sigma2**2)) - log(sqrt(2*M_PI)) - 0.5*log(sigma1**2 + sigma2**2)
