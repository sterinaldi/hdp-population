...
def main():
    ...
    parser.add_option("--hyperpars", type = "string", dest = "hyperpars", help = "MF hyperparameters (a0, b0, V0). See https://www.cs.ubc.ca/~murphyk/Papers/bayesGauss.pdf sec. 6 for reference", default = '1,3,0.25')
    parser.add_option("--hyperpars_ev", type = "string", dest = "hyperpars_ev", help = "Event hyperparameters (a0, b0, V0)")
    ...
    sampler = DPGMM.CGSampler(events = events,
                              ...
                              delta_M = options.deltaM,
                              m_min = options.mmin,
                              m_max = options.mmax,
                              verbose = options.verbose,
                              output_folder = options.output,
                              initial_cluster_number = options.initial_cluster_number,
                              process_events = options.process_events,
                              n_parallel_threads = options.n_parallel_threads,
