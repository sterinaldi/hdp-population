...
spec = [
    ('samples', types.float32[:]),
    ('mass_b', types.ListType(types.float32)),
    ('n_draws', types.int32),
    ('burnin', types.int32),
    ('step', types.int32),
    ('alpha0', types.float32),
    ('gamma', types.float32),
    ('sigma_b', types.ListType(types.float32)),
    ('output_folder', types.unicode_type),
    ('n_resamples', types.int32),
    ('delta_M', types.float32),
    ('verbose', types.int32),
    ('diagnostic', types.int32)
    ]
...
class gibbs_sampler:
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            :float alpha0:              Concentration parameter for inner DP (DPGMM).
            :float gamma:               Concentration parameter for outer DP (mass function).
            ...
            :double delta_s:          Interval width around s_old for tentative sample drawing.
        ...
        self.delta_s     = delta_s     # interval around old M sample (for updating)
    ...
    def update_table(self, sample_index, event_index):
        ...
        if uniform() < self.alpha0/(self.alpha0 + len(self.tables[event_index])):
            ...
            if uniform() < self.gamma/(self.gamma+len(self.components)):
                ...
                new_component     = [draw_mass(old_component[0], self.delta_M), draw_sigma(old_component[1], self.delta_s)]
    ...
    def evaluate_probability_t(self, table, component, component_index, sample_index, event_index, old_f, new_f):
        ...
        """
        if component[0] > 50 or component[1] > 20 or component[0] < 5 or component[1] < 1:
            return -np.inf
        n = self.table_index[event_index].count(table)
        if n == 0:
            return np.log(self.alpha0) + (self.evaluate_probability_sample(self.samples[event_index][sample_index], old_f, new_f))
        else:
            return np.log(n) + self.log_normal_density(self.samples[event_index][sample_index], *component)
    ...
    def evaluate_probability_component(self, component, component_index, event_index, sample_array):
        ...
        if n == 0:
            return np.log(self.gamma) + np.sum([np.log(self.samples_prior(x)) for x in sample_array])
        else:
            return np.log(n) + np.sum([self.log_normal_density(x, *component) for x in sample_array])
    ...
    def evaluate_probability_sample(self, sample, old_f, new_f):
        ...
        """
        # mettere LOGSUMEXP
        return np.log((np.sum([self.normal_density(sample, *self.components[index]) for table in self.tables for index in table]) + new_f - old_f + self.gamma * self.samples_prior(sample))/(np.sum([len(t) for t in self.tables])+self.gamma))
    ...
    def save_posterior_samples(self):
        ...
        """
        print('components = {0}'.format(len([self.components[index] for table in self.tables for index in table])))
        
        for table_is, table, i in zip(self.table_index, self.tables, range(len(self.posterior_samples))):
            dict = {}
            for index in range(len(table)):
                dict[index] = {'mean':self.components[table[index]][0], 'sigma': self.components[table[index]][1], 'weight': table_is.count(index)/len(table_is)}
            self.posterior_samples[i].append(dict)
    ...
    def run_sampling(self):
        ...
        if self.verbose:
            print('\n', end = '')
        # flattening
        for event_samples in self.posterior_samples:
            for sample in event_samples:
                for key in sample.keys():
                    try:
                        self.mass_samples[key].append(sample[key]['mean'])
                        self.sigma_samples[key].append(sample[key]['sigma'])
                    except:
                        self.mass_samples[key]  = []
                        self.sigma_samples[key] = []
                        self.mass_samples[key].append(sample[key]['mean'])
                        self.sigma_samples[key].append(sample[key]['sigma'])
        return
    ...
    def plot_samples(self):
        ...
        for samples, post, i in zip(self.samples, self.posterior_samples, range(len(self.samples))):
            fig = plt.figure()
            ...
            probs = []
            for a in app:
                probs.append([logsumexp([self.log_normal_density(a, sample[key]['mean'], sample[key]['sigma']) + np.log(sample[key]['weight']) for key in sample.keys()]) for sample in post])
    ...
    def compute_autocorrelation(self):
        ...
        """
        sq_mean = np.mean(self.mass_samples[0])**2
        sq_std  = np.std(self.mass_samples[0])**2
        self.autocorrelation = []
        for i in range(int(len(self.mass_samples[0])/2.)):
            ac = 0.
            for j in range(len(self.mass_samples[0])):
                try:
                    ac += (self.mass_samples[0][j] * self.mass_samples[0][j+i]) - sq_mean
                except:
                    ac += (self.mass_samples[0][j] * self.mass_samples[0][(j+i) - len(self.mass_samples[0])]) - sq_mean
            self.autocorrelation.append(ac/(sq_std*len(self.mass_samples[0])))
